...
const Bin: {[a: string]: () => void} = {
  help: function() {
    console.log(" - Usage:");
    console.log('\x1b[32m',"npx ts-node durinn dump-models","\x1b[0m","            Convert database (config/config.json) into models on ./classes folder");
    console.log('\x1b[32m',"npx ts-node durinn ts-model",'\x1b[31m[path | \'*\']\x1b[0m',"\x1b[0m","  Convert Sequelize model rawAttributes into Typescript attributes");
  },
  dump_models: async function() {
    console.log(" - Dumping database...");
    exec(`npx sequelize-auto -o "./.durinn/models" -d ${config['database']} -h ${config['host']} -u ${config['username']} -p ${config['port']} ${config['password'] ? ' -x ' + config['password'] : ''} -e ${config['dialect']} -z`, (err: any, stdout: any, stderr: any) => {
      if (err) {return console.log('\x1b[31m%s\x1b[0m', err);}

      const base_model = fs.readFileSync('./.durinn/base/ts-model.txt', 'utf8');
      const definitions = fs.readFileSync('./.durinn/models/db.d.ts', 'utf8');

      fs.readdirSync('./.durinn/models').forEach((file: string) => {
        if(file != 'db.d.ts' && file != 'db.tables.ts'){
          const durinn_file = './.durinn/models/' + file;
          const new_file = './classes/' + file;
          ...
          const durinn_content = fs.readFileSync(durinn_file, 'utf8');

          if (fs.existsSync(new_file)) {
            console.log('\x1b[32m',file,"\x1b[31m","File already exists");
          }else{

            let content = base_model.replace(new RegExp('{{MODEL-NAME}}', 'g'), MODEL_NAME);

            // GET INIT
            let firstvariable = `, \\{`;
            let secondvariable = `tableName\\: \\'` + file.substr(0, file.length - 3) + `\\'`;
            let pattern = firstvariable+"([\\s\\S]*?)"+secondvariable;
            ...
            {
              content = content.replace('{{MODEL-INIT}}', testRE[0].substr(2));
            }

            // GET definitions
            firstvariable = file.substr(0, file.length - 3) + `Attribute {`;
            secondvariable = `}`;
            pattern = firstvariable+"([\\s\\S]*?)"+secondvariable;
            ...
            console.log('\x1b[32m',file,"\x1b[0m","Created");
          }
        }
      });
    });
...
if(process.argv.length && process.argv[1].lastIndexOf('durinn') == process.argv[1].length - 6){
